...
export class Attachments {
    ...
    parse(attachments, builders) {
        ...
        const parsedAttachments = attachments.map(({ type, photo, video, link, doc, audio, poll, album, textlive, market }) => {
            switch (type) {
                case PHOTO: {
                    const { sizes } = photo;
                    if (sizes) {
                        if (!builder.image) {
                            ...
                            builders.push(this.createImageEmbed(this.popAttachment(sizes)));
                        }
                    }
                    else {
                    ...
                    if (ext === "gif") {
                        if (!builder.image) {
                            ...
                            if (builders.length < 10) {
                                builders.push(this.createImageEmbed(`attachment://${title}`)
                                    .attachFiles([
                                    new MessageAttachment(url, title)
                                ]));
                            }
